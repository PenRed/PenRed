...
def createTallyCylDoseDistrib(f, name, output, rmax, nr, zmin, zmax, nz, nphi):
    ...
    f.write(f"tallies/{name}/rmax {rmax}\n")
    f.write(f"tallies/{name}/nbinsr {nr}\n")
    f.write(f"tallies/{name}/zmin {zmin}\n")
    f.write(f"tallies/{name}/zmax {zmax}\n")
...
def createTallyImpactDetector(f, name, output, det,
    ...
    if age:
        ...
        f.write(f"tallies/{name}/age-min {agemin}\n")
        f.write(f"tallies/{name}/age-max {agemax}\n")
    else:
        f.write(f"tallies/{name}/age false\n")

    if fluence or spectrum or edep:
        f.write(f"tallies/{name}/emin {emin}\n")
        f.write(f"tallies/{name}/emax {emax}\n")
...
def createTallySpatialDoseDistrib(f, name, output,
    ...
    f.write(f"tallies/{name}/xmin {xmin}\n")
    f.write(f"tallies/{name}/xmax {xmax}\n")
    f.write(f"tallies/{name}/nx {nx}\n")
    f.write(f"tallies/{name}/xmin {ymin}\n")
    f.write(f"tallies/{name}/xmax {ymax}\n")
    f.write(f"tallies/{name}/nx {ny}\n")
    f.write(f"tallies/{name}/xmin {zmin}\n")
    f.write(f"tallies/{name}/xmax {zmax}\n")
    f.write(f"tallies/{name}/nx {nz}\n")
    if output:
        f.write(f"tallies/{name}/outputdir \"{output}\"\n")
    f.write("\n")

def createTallySphericalDoseDistrib(f, name, output, rmax, nr, ntheta, nphi):
...
def createTallyKerma(f, name, output,
    ...
    if meshType == 0: # Cartesian
        ...
        f.write(f"tallies/{name}/cartesian/xmin {min1}\n")
        f.write(f"tallies/{name}/cartesian/xmax {max1}\n")
        
        f.write(f"tallies/{name}/cartesian/ny {n2}\n")
        f.write(f"tallies/{name}/cartesian/ymin {min2}\n")
        f.write(f"tallies/{name}/cartesian/ymax {max2}\n")

        f.write(f"tallies/{name}/cartesian/nz {n3}\n")
        f.write(f"tallies/{name}/cartesian/zmin {min3}\n")
        f.write(f"tallies/{name}/cartesian/zmax {max3}\n")
        
    elif meshType == 1: # Cylindrical
        f.write(f"tallies/{name}/cylindrical/nr {n1}\n")
        f.write(f"tallies/{name}/cylindrical/rmin 0.0\n")
        f.write(f"tallies/{name}/cylindrical/rmax {max1}\n")
        
        f.write(f"tallies/{name}/cylindrical/nphi {n2}\n")

        f.write(f"tallies/{name}/cylindrical/nz {n3}\n")
        f.write(f"tallies/{name}/cylindrical/zmin {min3}\n")
        f.write(f"tallies/{name}/cylindrical/zmax {max3}\n")
        
    elif meshType == 2: # Spherical
        f.write(f"tallies/{name}/spherical/nr {n1}\n")
        f.write(f"tallies/{name}/spherical/rmin 0.0\n")
        f.write(f"tallies/{name}/spherical/rmax {max1}\n")
...
def createTallySpatialDistrib(f, name, output, det,
    ...
    f.write(f"tallies/{name}/detector {nx}\n")
    
    f.write(f"tallies/{name}/spatial/nx {nx}\n")
    f.write(f"tallies/{name}/spatial/xmin {xmin}\n")
    f.write(f"tallies/{name}/spatial/xmax {xmax}\n")
    f.write(f"tallies/{name}/spatial/ny {ny}\n")
    f.write(f"tallies/{name}/spatial/ymin {ymin}\n")
    f.write(f"tallies/{name}/spatial/ymax {ymax}\n")
    f.write(f"tallies/{name}/spatial/nz {nz}\n")
    f.write(f"tallies/{name}/spatial/zmin {zmin}\n")
    f.write(f"tallies/{name}/spatial/zmax {zmax}\n")

    f.write(f"tallies/{name}/energy/nbins {ebins}\n")
    f.write(f"tallies/{name}/energy/emin {emin}\n")
    f.write(f"tallies/{name}/energy/emax {emax}\n")
...
def createTallyAngularDet(f, name, output, det, emin, emax, ebins,
    ...
    f.write(f"tallies/{name}/theta1 {theta1}\n")
    f.write(f"tallies/{name}/theta2 {theta2}\n")
    f.write(f"tallies/{name}/phi1 {phi1}\n")
    f.write(f"tallies/{name}/phi2 {phi2}\n")
    if logSale:
