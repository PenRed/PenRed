...
namespace penred{
  ...
  namespace penMaterialCreator{
    ...
    struct PenelopeMats{
      static constexpr const std::array<const char*, 280> names = {
      "Hydrogen",
      "Helium",
      "Lithium",
      "Beryllium",
      ...
      static inline std::vector<std::string> matList() {
	return std::vector<std::string>(names.cbegin(), names.cend());
      }
      
      static inline unsigned getID(const std::string& name){
    ...
    inline int createMat(const std::string& name,
      ...
      if(dataBase.compare("PENELOPE") == 0){
	//Use the predefined materials in PENELOPE DB
	std::string filename;
	if(filenameIn.empty())
	  filename = name + ".mat";
	else
	...
	return createMat(PenelopeMats::getID(dataBaseMat),		  
			 filename,
			 errorString);
      }
