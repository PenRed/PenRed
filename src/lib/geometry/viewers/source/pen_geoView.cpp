...
namespace penred{
  ...
  namespace geometry{
    ...
    bool Viewer::checkStep(pen_particleState& state,
			   double toTravel,
			   viewError& err) const{
    ...
    int Viewer::init(const pen_parserSection& configIn,
	...
	if(verbose > 0){
	  printf("penred:geometry:Viewer: init: Error: field 'type' not specified. "
		 "String expected.\n");
	}
	return -1;
      }
      ...
      if(!geometry){
	if(verbose > 0){
	  printf("penred:geometry:Viewer: init: Error creating geometry instance "
		 "of type '%s'\n", geoType.c_str());
	}
	return -2;
      }
      ...
      //Check errors
      if(geometry->configureStatus()){
	if(verbose > 0)
	  printf("%s\n", geometry->configureStatus().stringify().c_str());
	return -3;
      }

      if(verbose > 2){
	printf("penred:geometry:Viewer: init: Geometry load successfully\n");
      }
  
      return 0;
    }

    void Viewer::testX(std::vector<viewError>& errors,
    ...
    void Viewer::testY(std::vector<viewError>& errors,
		       const float x, const float y, const float z,
    ...
    void Viewer::testZ(std::vector<viewError>& errors,
		       const float x, const float y, const float z,
    ...
    void Viewer::set3DResolution(const unsigned nx, const unsigned ny,
				 const float dx, const float dy,
